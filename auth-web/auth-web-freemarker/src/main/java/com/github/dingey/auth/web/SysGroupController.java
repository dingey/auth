...
public class SysGroupController {
    ...
    public AuthResult<Integer> save(@Validated(Insert.class) SysGroup group) {
        int save = sysGroupService.save(group);
        return AuthResult.success(save);
    }

    @ResponseBody
    @PostMapping(path = "/sys/group/update")
    public AuthResult<Integer> update(@Validated(Update.class) SysGroup group) {
        int update = sysGroupService.update(group);
